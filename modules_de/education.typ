// education.typ
#import "../helpers.typ": *
#import "@preview/brilliant-cv:3.1.2": h-bar

// #cv-section("Abschlüsse", tags: ("job", "Abschlüsse"))

#cv-section("Bildungsweg", tags: ("education", "IT", "Lehre", "job"))

#cv-entry(
  title: [Diplom-Wirtschaftsinformatiker (FH)],
  society: [SRH Hochschule, Heidelberg],
  date: [2003 - 2009],
  location: [Heidelberg, DE],
  description: list(
    [Diplomarbeit (Note 1,7): "Konzepte zur Integration von Business-Daten in Office-Software mit SAP" (bei IBM SAP International Competence Center)],
    [2003 – 2005: Studium der Angewandten Informatik (48/120 LP) | Universität Heidelberg],
    [Studienschwerpunkte: Software-Qualitätssicherung, Anwendungsentwicklung, Datenbanken]
  ),
  tags: ("edu_it", "IT", "Architecture", "education")
)

#cv-entry(
  title: [Studium Grundschullehramt],
  society: [Pädagogische Hochschule Heidelberg],
  date: [1996 - 2000],
  location: [Heidelberg, DE],
  description: list(
    [Fächer: Mathematik, Musik und pädagogische Psychologie],
    [Abschluss von 6 Schulpraktika als Lehrer an staatlichen und internationalen Schulen]
  ),
  tags: ("edu_lehre", "Lehre", "Pädagogik", "education")
)

#cv-entry(
  title: [Technisches Abitur],
  society: [Ferdinand-von-Steinbeis-Schule],
  date: [1991 - 1994],
  location: [Reutlingen, DE],
  tags: ("edu_it", "education")
)

#cv-entry(
  title: [Fachschule für Elektrotechnik],
  society: [Ferdinand-von-Steinbeis-Schule],
  date: [1989 - 1991],
  location: [Reutlingen, DE],
  description: list([Fundament der elektrischen Energietechnik und industriellen Elektronik.]),
  tags: ("EE", "Hardware")
)

